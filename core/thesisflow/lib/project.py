...
class ProjectManager:
    ...
    def zotero_service(self):
        if not hasattr(self, '_zotero_service') or self._zotero_service is None:
            ...
            # scripts/lib/project.py -> scripts/lib -> scripts -> thesisflow-manager
    ...
    def sync_zotero(self, project_dir: Path) -> dict:
        ...
        if not collection_id:
            return {"success": False, "message": "No ZOTERO_COLLECTION_ID found in thesisflow.json"}
    ...
    def _load_config(self, project_dir: Path) -> dict:
        config_path = project_dir / "thesisflow.json"
    ...
    def init_project(self, name: str) -> Path:
        ...
        if (project_dir / "thesisflow.json").exists():
            raise FileExistsError(f"A ThesisFlow project already exists at {project_dir}")
        ...
        self._setup_agent_config(project_dir)
        return project_dir

    def _setup_agent_config(self, project_dir: Path):
        """Creates .agent/rules and .agent/workflows with default configuration."""
        ...
        # 1. Create Rule: thesisflow.md
        rule_path = rules_dir / "thesisflow.md"
        if not rule_path.exists():
            # Resolve SKILL.md path relative to this script
            # project.py is in scripts/lib/
            # SKILL.md is in root (../../SKILL.md)
            skill_md_path = (Path(__file__).parent.parent.parent / "SKILL.md").resolve().as_posix()
            
            rule_content = f"""---
trigger: always_on
---

# ThesisFlow Agent Rules

You are working on a ThesisFlow project.
...
"""
            rule_content = rule_content.replace("\\", "/") # Ensure forward slashes for paths
            rule_path.write_text(rule_content, encoding="utf-8")

        # 2. Create Workflows
        
        # Setup Workflow
        setup_wf_path = workflows_dir / "setup.md"
        if not setup_wf_path.exists():
            setup_wf_path.write_text("""---
...
""", encoding="utf-8")
        ...
        # Draft Workflow
        draft_wf_path = workflows_dir / "draft.md"
        if not draft_wf_path.exists():
            draft_wf_path.write_text("""---
...
""", encoding="utf-8")
        ...
        # Research Workflow
        research_wf_path = workflows_dir / "research.md"
        if not research_wf_path.exists():
            research_wf_path.write_text("""---
...
""", encoding="utf-8")
        ...
        # Audit Workflow
        audit_wf_path = workflows_dir / "audit.md"
        if not audit_wf_path.exists():
            audit_wf_path.write_text("""---
...
""", encoding="utf-8")
        ...
        # Production Workflow
        prod_wf_path = workflows_dir / "production.md"
        if not prod_wf_path.exists():
            prod_wf_path.write_text("""---
...
""", encoding="utf-8")
    ...
    def add_chapter(self, project_dir: Path, title: str) -> Path:
        ...
        existing_chapters = [x for x in project_dir.iterdir() if x.is_dir() and not x.name.startswith('.') and x.name != 'assets']
        next_idx = len(existing_chapters) + 1
        prefix = f"{next_idx:02d}_"
        folder_name = f"{prefix}{safe_title}"
        chapter_path = project_dir / folder_name
    ...
    def validate_structure(self, project_dir: Path, fix: bool = False) -> List[str]:
        ...
        for chap_name in original_order:
            if not (project_dir / chap_name).exists():
        ...
        ignored = {'.', 'assets', '.build', '__pycache__', 'references.bib', 'README.md', 'thesisflow.json'}
        orphan_chapters = []
        for item in project_dir.iterdir():
            if item.is_dir() and item.name not in ignored and not item.name.startswith('.'):
                 if item.name not in original_order:
                      msg = f"ORPHAN_CHAPTER: '{item.name}' exists but is not in thesisflow.json order."
                      issues.append(msg)
    ...
    def rename_chapter(self, project_dir: Path, old_id: str, new_name: str) -> Path:
        """
        Renames a chapter folder and updates thesisflow.json.
    ...
    def delete_chapter(self, project_dir: Path, chapter_id: str, force: bool = False) -> str:
        """
        Deletes a chapter folder and updates thesisflow.json.
